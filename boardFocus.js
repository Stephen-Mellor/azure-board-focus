try {
  ...
  if (body.classList.contains("azure-board-focus--normal")) {
    ...
    // Create views
    var initialBoard = document.getElementsByClassName(
      "flex-row flex-grow v-scroll-auto"
    )[0];
    ...
    var revampedKanbanHeader = revampedBoard.getElementsByClassName(
      "kanban-board-column-header-container"
    )[0];
    var revampedKanbanContent = revampedBoard.getElementsByClassName(
...
function getNormal() {
  return document.getElementsByClassName(
    "azure-board-focus-content--normal"
  )[0];
}

function getFocused() {
  return document.getElementsByClassName(
    "azure-board-focus-content--focused"
  )[0];
}

function getRevamped() {
  return document.getElementsByClassName(
    "azure-board-focus-content--revamped"
  )[0];
}
...
function expandBoardColumns() {
  ...
  Array.from(
    document.getElementsByClassName("kanban-board-row-container")[0].children
  ).forEach((row, i) => {
  ...
  body.setAttribute(
    "data-indicesOfExpanded",
    JSON.stringify(indicesOfExpanded)
  );
  window.addEventListener("beforeunload", unexpandBoardColumns, { once: true });
}

function unexpandBoardColumns() {
  var needsUnexpanding = JSON.parse(
    body.getAttribute("data-indicesOfExpanded")
  );
  Array.from(
    document.getElementsByClassName("kanban-board-row-container")[0].children
  ).forEach((row, i) => {
